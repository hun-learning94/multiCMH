...
void order_nodes(const int N, const int *const merge, const t_index *const node_size, int *const order)
{
  ...
  */
  ...
  do
  {
    ...
    child = merge[parent];
    if (child < 0)
    { // singleton node, write this into the 'order' array.
      order[pos] = -child;
      ++pos;
    }
    else
    { /* compound node: put it on top of the queue and decompose it
...
enum hclust_fast_methods
{
  ...
  // ward linkage (not included in the original https://github.com/cdalitz/hclust-cpp)
